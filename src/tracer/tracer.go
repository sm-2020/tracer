...
func socketAddr() ([4]byte, error) {
    ...
    err = errors.New("Error connecting to the internet.")
    return socketAddr,err
}

func getDestAddr(dest string) ([4]byte, error) {
...
func Tracer(options *cli.Context) error {
    ttl :=options.Int("hop")
    ...
    destAddr, err := getDestAddr(options.Args().Get(0))
	if err != nil {
		return err
	}
    ...
    fmt.Printf("tracing [%v] - %v with packetSize=%v,maxHops=%v,startHop=%v,timeout=%v\n\n",
        ...
        for {
            ...
            if err == nil {
                retries = 0
                hop := Hop{Status: true, Addr: from, Num: n, Duration: elapsed, TTL: ttl}
                if hop.GetIP() == destAddrString || ttl >= options.Int("maxhops") {
                    break
                }
                ttl += 1
                fmt.Printf("\n%v. %v // [%v] // %v", hop.TTL, hop.Domain(), hop.GetIP(), hop.Duration)
            } else  {
                hop := Hop{Status: false, Num: n, Duration: elapsed, TTL: ttl}
